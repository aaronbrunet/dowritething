...
export const EditForm = (props) => {
    ...
    <div className='entry-form'>
      ...
      <button onClick={handleSubmit} className="items-center shadow bg-spring-wood-800 text-white text-xs rounded px-4 py-2 hover:text-spring-wood-800 hover:bg-white">Add{name ? ` '${name}' ` : ' '}as new project</button>
      <button onClick={setEditing} className="inline-flex items-center shadow bg-white text-spring-wood-800 text-xs rounded px-4 py-2 hover:white hover:text-spring-wood-800">Cancel</button>
      </>)}
      {props.editType === 'edit' && (<>
      {interpretFields(input,model,flag)}
      <div className="flex flex-row">
        <button className="inline-flex items-center shadow bg-spring-wood-800 text-white text-xs rounded px-4 py-2 hover:text-spring-wood-800 hover:bg-white">
          Edit</button>
        <button onClick={setEditing} className="inline-flex items-center shadow bg-spring-wood-800 text-white text-xs rounded px-4 py-2 hover:text-spring-wood-800 hover:bg-white">
          Cancel</button>
      </div>
      </>)    
      }
